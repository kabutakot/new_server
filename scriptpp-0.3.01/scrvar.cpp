...
const int size_of_memblock_header = sizeof(int) * 2;
...
{
    if(!o2.p) return o2; // it is invalid, return the invalid str...
...
{
    if(!p || !o2.p) return 0;  // hmm... got a better idea?
...
{
    if(IsInvalid()) return ScriptVariable(-1);
...
ScriptVariable::Substring ScriptVariable::Strchr(int c)
{
    ...
    if(!t) return ScriptVariable::Substring(); // invalid
...
{
    if(!master->IsValid()) return false;
...
bool ScriptVariable::GetLongLong(long long &l) const /* throw () */
{
    if(!p) return false;
    long long ret;
    char *err;
...
{
    // if(!p) /**/ // no need for this, subseq. call will do
    long ret;
    if(GetLong(ret))
        return ret;
    else
...
long long ScriptVariable::GetLongLong() const
...
double ScriptVariable::GetDouble() const
                      /* throw (ScriptVariable::BadNumber) */
{
    // if(!p) /**/ // no need for this, subseq. call will do
    double ret;
